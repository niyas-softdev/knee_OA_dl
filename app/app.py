...
st.write("""
Upload a knee X-ray image to classify the severity of osteoarthritis 
using a deep learning model (Xception) trained on KL grades 0 to 4.
""")
...
with st.sidebar:
    st.image(icon)
    st.subheader("KNEE_OA_DL")
    st.caption("=== Bone Doctor AI ===")
...
if uploaded_file is not None:
    with col1:
        ...
        if st.button(":arrows_counterclockwise: Predict Arthrosis in the Knee"):
            ...
            number = np.where(y_pred == probability)
